async function fetchData() {
    try {
        ...
        document.getElementById('data-table-body').innerHTML = '<tr><td colspan="9">Failed to load data. Please check the console for errors.</td></tr>';
        return null;
    }
  }
  ...
  function filterData(data, filters) {
    return data.filter(item => {
        ...
        if (filters.age !== 'all' && item.Age !== filters.age) return false;
  ...
  function updateDataTable(filteredData) {
    ...
    if (filteredData && filteredData.length > 0) { // Only update if there's data
